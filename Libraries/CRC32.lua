...
local band=bit32.band or load([[
	return function(a,b)
		return a&b
	end
	]])()

local bor=bit32.bor or load([[
	return function(a,b)
		return a|b
	end
	]])()
...
function CRC32.hash(str)
	...
	for i=1,count do
		...
		local a = band(rshift(crc,8),0x00FFFFFF)
		local b = tab[band(xor(crc,byte),0xFF)+1]
		crc =  xor(a , b)
	end
	crc = xor(crc , 0xFFFFFFFF)
	crc = bor(crc,-band(crc,lshift(1,31)))
