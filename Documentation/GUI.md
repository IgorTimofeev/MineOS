...
> Создать объект типа "контейнер". Как было сказано выше, каждый контейнер - это объект-группировщик для других объектов, перечисленных ниже. К примеру, при изменении позиции контейнера на экране все его дочерние элементы будут также смещены на соответствующие координаты. Контейнер также содержит все основные методы по добавлению дочерних элементов (виджетов) и работе с ними.

> Для получения доступа к дочерним элементам контейнера достаточно обратиться к нумерически индексированной таблице *контейнер.children*. Чем больше индекс дочернего элемента, тем "ближе" он будет отрисован к глазам пользователя на экране.

>Все дочерние элементы контейнера имеют свою *localPosition* (к примеру, *{x = 4, y = 2}*) в контейнере, а при добавлении нового элемента в контейнер используются именно локальные координаты.  Для получения глобальных (экранных) координат дочернего элемента необходимо обращаться к *element.x* и *element.y*. Подчеркиваю, что глобальная (экранная) позиция дочерних элементов рассчитывается при каждой отрисовке содержимого контейнера. Таким образом,  изменяя глобальные координаты дочернего элемента вручную, вы, в сущности, ничего не добьетесь.
...
| *int* | width | Ширина объекта |
| *int* | height | Высота объекта |
> Создать объект типа "окно" для дальнейшей работы. Каждое окно - это наследник объекта объекта типа "контейнер" (см. выше), содержащий методы обработки системных событий и возврата данных окна.

| Свойство объекта |Описание |
| ------ | ------ |
| :**handleEvents**([*int* timeout]) | Запустить обработчик событий и ожидать действий со стороны пользователя. К примеру, при нажатии на кнопку на экране система автоматически определит "нажатый" элемент и осуществит отрисовку. Опциональный аргумент timeout эквивалентен аналогичному аргументу в *computer.pullSignal(timeout)* |
| .**onTouch**(*table* eventData) | Метод, вызывающийся при каждом событии типа *touch* |
| .**onDrag**(*table* eventData) | Метод, вызывающийся при каждом событии типа *drag* |
| .**onScroll**(*table* eventData) | Метод, вызывающийся при каждом событии типа *scroll* |
| .**onKeyDown**(*table* eventData) | Метод, вызывающийся при каждом событии типа *key_down* |
| .**onAnyEvent**(*table* eventData) | Метод, вызывающийся при любом событии для дальнейшей обработки разработчиком |
| .**onDrawStarted**() | Метод, вызывающийся до начала отрисовки содержимого окна в экранный буфер |
| .**onDrawFinished**() | Метод, вызывающийся после отрисовки содержимого окна в экранный буфер |
...
| ------ | ------ |
| :**draw**() | Обязательный метод, вызываемый для отрисовки виджета на экране. Он может быть определен пользователем любым удобным для него образом. Повторюсь, что данный метод осуществляет отрисовку только в экранный буфер, а не на экран. |
| :**isClicked**( *int* x, *int* y ) | Метод для проверки валидности клика на объект. Используется родительскими методами контейнеров и удобен для ручной проверки пересечения указанных координат с расположением объекта на экране |

После добавления виджета в контейнер с помощью метода *:addChild* он приобретает дополнительные свойства для удобства использования:
...
| *boolean* | .**isHidden** | Указатель на таблицу-контейнер родителя этого виджета |
...
| Свойство объекта |Описание |
| ------ | ------ |
| .**onTouch**( *table* eventData )| Метод, вызываемый после нажатия кнопки в обработчике событий |
| :**press**()| Изменить состояние кнопки на "нажатое" |
...
| Свойство объекта |Описание |
| ------ | ------ |
| .**onTouch**( *table* eventData )| Метод, вызываемый после нажатия на лейбл в обработчике событий |
...
| Свойство объекта |Описание |
| ------ | ------ |
| :**addItem**( *string* text, *boolean* disabled, *string* shortcut, *int* color )| Добавить в комбо-бокс элемент с указанными параметрами |
...
| Свойство объекта |Описание |
| ------ | ------ |
| .**onTouch**( *table* eventData )| Метод, вызываемый после нажатия на изображение в обработчике событий |
...
| Свойство объекта |Описание |
| ------ | ------ |
| .**onTouch**( *table* eventData )| Метод, вызываемый после нажатия на текстбокс в обработчике событий |
...
| Свойство объекта |Описание |
| ------ | ------ |
| .**onFileSelected**( *int* currentFile )| Метод, вызываемый после выбора файла в TreeView |
