	...
	if file then
		...
		if readedSignature == OCIFSignature then
			...
			if encodingMethods.load[encodingMethod] then
				...
				file:close()
				error("Failed to load OCIF image from path (\"" .. tostring(path) .. "\"): encoding method \"" .. tostring(encodingMethod) .. "\" is not supported")
			end
		else
			file:close()
			error("Failed to load OCIF image from path (\"" .. tostring(path) .. "\"): wrong signature (\"" .. tostring(readedSignature) .. "\")")
		end
	else
