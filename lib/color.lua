...
local mathFloor, mathMax, mathMin, mathHuge, mathModf = math.floor, math.max, math.min, math.huge, math.modf
local IntegerToRGB, RGBToInteger, blend, transition
...
if computer.getArchitecture and computer.getArchitecture() == "Lua 5.3" then
	IntegerToRGB, RGBToInteger, blend, transition = load([[
		return
			...
			function(firstColor, secondColor, transparency)
				local r1, g1, b1, r2, g2, b2, invertedTransparency = firstColor >> 16, firstColor >> 8 & 0xFF, firstColor & 0xFF, secondColor >> 16, secondColor >> 8 & 0xFF, secondColor & 0xFF, 1 - transparency
	...
	IntegerToRGB, RGBToInteger, blend, transition = load([[
		...
		return
			...
			function(firstColor, secondColor, transparency)
				local invertedTransparency, r1, g1, b1 = 1 - transparency, IntegerToRGB(firstColor)
...
local function to8Bit(color24Bit)
	local closestDelta, r, g, b, closestIndex, delta, paletteR, paletteG, paletteB = mathHuge, IntegerToRGB(color24Bit)

	for index = 1, #palette do
		if color24Bit == palette[index] then
			return index - 1
		else
			paletteR, paletteG, paletteB = IntegerToRGB(palette[index])
