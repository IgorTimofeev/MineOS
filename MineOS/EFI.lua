...
local function getBootableDrives()
	local dr = {}
	for address in ls("filesystem") do
		local fs = pr(address)
		if fs.exists("/init.lua") or fs.exists("/MineOS/EFI.lua") then table.insert(dr, fs.address) end
	end
	return dr
end
...
local function waitForAlt(t, dr)
	...
	local a = pr(ee.getData() or "")
	bt(a and a.address or dr[1])
end
