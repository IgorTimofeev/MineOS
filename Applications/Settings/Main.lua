...
local leftPanel = window:addChild(GUI.panel(1, 1, 30, 1, 0xE1E1E1))
window.backgroundPanel.localX = leftPanel.width + 1

window.actionButtons.localY = 2
window.actionButtons:moveToFront()

local modulesLayout = window:addChild(GUI.layout(1, 3, leftPanel.width, 1, 1, 1))
modulesLayout:setAlignment(1, 1, GUI.ALIGNMENT_HORIZONTAL_LEFT, GUI.ALIGNMENT_VERTICAL_TOP)
modulesLayout:setSpacing(1, 1, 1)

window.contentLayout = window:addChild(GUI.layout(window.backgroundPanel.localX, 1, 1, 1, 1, 1))


local function moduleDraw(object)
	local textColor = object.pressed and 0xE1E1E1 or 0x2D2D2D
	if object.pressed then
		buffer.drawRectangle(object.x, object.y, object.width, object.height, 0x336DFF, textColor, " ")
		buffer.drawText(object.x, object.y - 1, 0x336DFF, string.rep("▄", object.width))
		buffer.drawText(object.x, object.y + object.height, 0x336DFF, string.rep("▀", object.width))
	end
...
for i = 1, #modules do
	...
	if result then
		...
		if success then
			local object = modulesLayout:addChild(GUI.object(1, 1, modulesLayout.width, 3))
...
window.onResize = function(width, height)
	...
	window.contentLayout.width, window.contentLayout.height = window.backgroundPanel.width, window.backgroundPanel.height
