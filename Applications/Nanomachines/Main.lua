...
local configPath = MineOSPaths.applicationData .. "Nanomachines/Config.cfg"
...
local function addMessage(prefix, ...)
	...
	message = string.wrap(prefix .. table.concat(message, ", "), textBox.width - 2)
...
local function updateEffects(variants)
	...
	local function effectDraw(object)
		...
		buffer.drawText(object.x + 1, object.y + 1, object.textColor, object.text)
	end
